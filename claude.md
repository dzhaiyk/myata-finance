...
## Что это
Финансовый учёт заведения «Мята | Platinum 4YOU»: отчёты смен и касса, выписки банков, P&L и Cash Flow, зарплаты, счета, контроль замкнутого контура денег.
...
11. Значения, зависящие от заведения (бренд, юрлицо, отделы, типы оплат, пороги, ставки, валюта, язык, контрагенты, персональные данные), в код не пишутся — им место в настройках, справочниках, seed клиента или переводах. Смысл не определяется сравнением с отображаемым названием: отделы, роли и статьи сопоставляются по коду. Подробно — ADR-0010 и `docs/20-architecture/customization.md`.
12. Код записи справочника (отдела, статьи, роли) создаётся один раз — из названия или явным правилом — и после этого не редактируется ни в интерфейсе, ни вручную. На код ссылаются сохранённые данные: переименование безопасно, смена кода тихо рвёт связь со старыми записями. Поле кода в формах — только для чтения.
...
- Что специфично для «Мяты» и куда должно переехать → `docs/20-architecture/customization.md`
