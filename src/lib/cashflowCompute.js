// Cash Flow прямым методом: чистая функция, покрыта тестами (TASK-029, ADR-0009).
// До 05.09.2026 расчёт жил внутри CashFlowPage и проверялся только глазами
// (вопрос reporting #1). Логика перенесена дословно; изменено одно — месяц
// операции определяется по строке даты, а не через Date в поясе браузера.
import { isCapexRow, PAYROLL_CATEGORIES } from './config.js'
import { cashPayrollOf } from './reconcile.js'

// === CASH FLOW STRUCTURE ===
export const CF_STRUCTURE = [
...
export const PAYROLL_CATS = PAYROLL_CATEGORIES
export const COGS_CATS = ['cogs_kitchen', 'cogs_bar', 'cogs_hookah']
export const RENT_CATS = ['rent_premises', 'rent_warehouse', 'rent_property_tax']
export const UTIL_CATS = ['util_electric', 'util_water', 'util_heating', 'util_bi', 'util_internet', 'util_waste', 'util_other']
export const MKT_CATS = ['mkt_smm', 'mkt_target', 'mkt_2gis', 'mkt_yandex', 'mkt_google', 'mkt_other']
export const TAX_CATS = ['tax_retail', 'tax_payroll', 'tax_insurance', 'tax_alcohol', 'tax_hookah', 'tax_other']
...
export function computeMonthCF(targetYear, targetMonth, dailyReports, bankTx, pnlData, investorTx) {
  ...
  // Месяц — по строке даты: new Date('YYYY-MM-DD') даёт UTC-полночь, и в поясе
  // западнее UTC getMonth() вернул бы предыдущий месяц
  const ym = `${targetYear}-${String(targetMonth).padStart(2, '0')}`
  const inMonthDate = (d) => String(d ?? '').slice(0, 7) === ym
  const monthReports = dailyReports.filter(r => inMonthDate(r.report_date))
  ...
  const inMonth = (tx) => inMonthDate(tx.transaction_date)
  ...
  const monthInvTx = (investorTx || []).filter(t => inMonthDate(t.transaction_date))
